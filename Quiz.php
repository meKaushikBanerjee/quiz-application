...
if(($time>=$qstime)&&($udate==$date)&&($time<=$qetime))
{
    ...
    if($query->rowCount() > 0)
    {
    ...
    else
   	{
	    ...
	    $sql="SELECT qq.QUESTION_NUMBER as qno,qq.QUESTION as qs,qq.OPTION1 as o1,qq.OPTION2 as o2,qq.OPTION3 as o3,qq.OPTION4 as o4,qq.QUIZ_NAME as qn,qm.TOTAL_QUESTIONS as totqn,qm.QUIZ_DURATION as qd,qm.UNANSWERED_QS_TIME as uqt from quizquestion qq join quizmaster qm on qq.QUIZ_ID=qm.QUIZ_ID where qm.QUIZ_ID='$qid' and qq.QUIZ_ID='$qid' and qq.QUESTION_NUMBER='$cqno'";
	    $query = $dbh->prepare($sql);
	    $query->execute();
	    $results=$query->fetchAll(PDO::FETCH_OBJ);
	    if($query->rowCount() > 0)
...
?>
						...
						<p class="instruction">INSTRUCTIONS FOR THE QUIZ:</p>
						<div class="quiz-rules">
							<p>1. Duration of each question is <?php echo ($result->qd); ?> seconds .</p>
							<p>2. No. of questions are <?php echo ($result->totqn); ?>.</p>
							<p style="color: red; font-size: 25px;" class="stroke">3. If you missed or didn't answer any question, then don't worry, because after the initial submission of the quiz, you will get the remaining time left, untill the quiz ends, to submit your unanswered questions.</p>

							<p>4. In case of tie score, the winner shall be decided based on the submission time (fastest finger first) of the quiz.</p>
							<p>5. Each question carry equal marks.</p>
							<p>6. Correct answer to be chosen from the options given.</p>
							<p>7. Incomplete/Wrong filling of particulars shall lead to rejection of entry.</p>
							
							<p style="color: red; font-size: 25px;" class="stroke">8. Do not reload, or press browser back button or close this window while the quiz is on. Otherwise your quiz will get cancelled and your answers will not be submitted!</p>
						</div>

						<form id="q" class="form-horizontal center" action="QuizAction.php" method="POST">
							<div class="form-group">
								<div class="row">        
									<div class="col-sm-offset-2 col-sm-11">
									    <input type="submit" value="Start" name="gotoquiz">
									</div>
								</div>
							</div>
						</form>
<?php
				...
				elseif((($result->qno) == $cqno)&&($_SESSION['check'] !=0))
				{
